...
// ->  ({ ru : string, en: string } * slug) list

// type Tag = Tag of { ru : string, en: string } * slug


// fetchFunction :: str -> Async ((displayValue, slug) list)

type AcItem = { DisplayValue: string; Slug: string }
...
let private filterItems (query: string) (items: string list) =
    ...
    | q ->
        items
        |> List.filter (fun x -> x.ToLower().Contains(q))
...
let view (props: Props) =
    ...
    let getTags url query =
        async {
            ...
            let items = decodeResponseAuto<string list> response
    ...
    let tagsReceived tags st =
        { st with
              DisplayedItemsDeferred = Resolved tags }
    ...
    let inputValueChanged x st =
        ...
        | str, ItemsList tags ->
            ...
            { st with
                  InputValue = str
                  DisplayedItemsDeferred = Resolved <| Ok filtered }
        | str, ItemsUrl url ->
            async {
            ...
            { st with
                  InputValue = str
                  DisplayedItemsDeferred = InProgress }
...
onChange: () => x[]
*)
    // update: sting -> state -> state * Cmd
    let itemClicked value state =
        props.OnItemSelected value // client code state 
        inputValueChanged "" state // local state
        // (state, Cmd (ParentEventConstructor value))
            ...
            | Resolved (Ok tags) ->
                div
                    ...
                    (tags
                     |> List.map (fun x ->
                         span [ Class "mm-autocomplete-item"
                                OnClick(const' x >> itemClicked >> state.update) ] [
                             str x
                         ]))
        ]
    ]
